...
RANDOM_THRESHOLD = {'dense': 0.4, 'medium': 0.6, 'sparse': 0.9, 'verysparse': 0.95}
...
class DataGenerator():
    ...
    #     np.savez(
    #         f"{type}_{adj_type}_d{str(data_dim)}_n{str(num_samples)}",
...
if __name__ == '__main__':
    ...
    gen = DataGenerator()
    adj_type = 'random_verysparse'
    data_dim = 15
    very_sparse_adj_mtx = gen.create_adjacency(d=data_dim, adj_type=adj_type)
    np.savez(f"./synth_data_files/very_sparse_adj_mtx_{data_dim}.npz", A=very_sparse_adj_mtx)
