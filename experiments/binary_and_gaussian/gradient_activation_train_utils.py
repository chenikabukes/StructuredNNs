...
def register_activation_gradients_hooks(model, gradient_norms_storage):
    """
    Registers hooks to capture gradients of the activations.
    The gradient norms will be stored in the gradient_norms_storage list.
    """

    def hook_fn(module, grad_input, grad_output):
        if isinstance(module, torch.nn.modules.activation.ReLU):
            gradient_norm = grad_output[0].norm(2)
            gradient_norms_storage.append(gradient_norm.item())
...
def train_loop(
    ...
    activation_gradient_norms_per_epoch = []

    for epoch in range(1, max_epoch + 1):
        model.train()  # Set model to training mode
        # Initialize list to store gradient norms for the current epoch
        activation_gradient_norms = []
        # Register hooks to capture activation gradients
        hooks = register_activation_gradients_hooks(model, activation_gradient_norms)
        ...
        epoch_activation_gradient_norm = np.mean(activation_gradient_norms)
        train_losses_per_epoch.append(epoch_train_loss)
        val_losses_per_epoch.append(epoch_val_loss)
        activation_gradient_norms_per_epoch.append(epoch_activation_gradient_norm)

        wandb.log({
            "epoch": epoch,
            "train_loss": epoch_train_loss,
            "val_loss": epoch_val_loss,
            "activation_gradient_norm": epoch_activation_gradient_norm
        })
    ...
    return {
        ...
        "gradient_norms_per_epoch": activation_gradient_norms
    }
