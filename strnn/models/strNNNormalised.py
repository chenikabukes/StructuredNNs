...
class MaskedLinear(nn.Linear):
    ...
    #     if len(dimensions) == 2:
    #         fan_in = dimensions[1]
    #         print("fan_in calc:", fan_in)
    #         fan_out = dimensions[0]
    #         print("fan_out calc:", fan_out)
    #     else:
...
class StrNNNormalised(nn.Module):
    """
    ...
    def __init__(
        ...
        # Create MaskedLinear and LayerNorm layers
        for h0, h1 in zip(hs[:-1], hs[1:-1]):
            self.net_list.append(MaskedLinear(h0, h1, ian_init, activation))
            self.net_list.append(nn.LayerNorm(h1))
            self.net_list.append(NONLINEARITIES[activation])

        self.net_list.append(MaskedLinear(hs[-2], hs[-1], ian_init, activation))
    ...
    def update_masks(self):
        ...
        for layer, mask in zip(layers, self.masks):
            print("mask for each layer", mask)
            layer.set_mask(mask)
